...
export const treeTemplateGenerator = function (fw, formConfig) {
  const wop = fw.options
  const typeAttr = `type=${wop.type}`
  const data = `data={fw.options.data}`
  ...
  const alertTemplate = `<el-tree ${data}  ${typeAttr}  ${closableAttr}  ${centerAttr} 
  ${showIconAttr} ${effectAttr}>
</el-tree>`
