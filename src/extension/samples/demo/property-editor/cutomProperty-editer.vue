<template>
    <el-form-item prop="cutomProperty" :rules="nameRequiredRule">
        ...
        <template v-if="(!!selectedWidget.category && (selectedWidget.type !== 'sub-form')) || noFieldList">
            <el-input type="text" v-model="optionModel.cutomProperty" :readonly="widgetNameReadonly"
                @change="updateWidgetNameAndRef"></el-input>
        </template>
        <template v-else>
            <el-select v-model="optionModel.cutomProperty" allow-create filterable :disabled="widgetNameReadonly"
                @change="updateWidgetNameAndRef" :title="i18nt('designer.setting.editNameHelp')">
                <el-option v-for="(sf, sfIdx) in serverFieldList" :key="sfIdx" :label="sf.label"
                    :value="sf.cutomProperty"></el-option>
            </el-select>
        </template>
    </el-form-item>
</template>
...
export default {
    ...
    data() {
        return {
            nameRequiredRule: [{ required: true, message: 'cutomProperty required' }],
        }
    },
    computed: {
        noFieldList() {
    ...
    methods: {
        updateWidgetNameAndRef(newName) {
            ...
            if (isEmptyStr(newName)) {
                this.selectedWidget.options.cutomProperty = oldName
        ...
        getLabelByFieldName(fieldName) {
            for (let i = 0; i < this.serverFieldList.length; i++) {
                if (this.serverFieldList[i].cutomProperty === fieldName) {
