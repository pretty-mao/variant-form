<template>
  <el-form-item prop="name" :rules="nameRequiredRule">
    ...
    <template v-if="(!!selectedWidget.category && (selectedWidget.type !== 'sub-form')) || noFieldList">
      <el-input type="text" v-model="optionModel.name" :readonly="widgetNameReadonly"
        @change="updateWidgetNameAndRef"></el-input>
    </template>
    <template v-else>
      <el-select v-model="optionModel.name" allow-create filterable :disabled="widgetNameReadonly"
  ...
  watch: {
    'optionModel.name': {
      handler: function (newVal, oldVal) {
        // this.updateWidgetNameAndRef(newVal)
        console.log('name-editor watch optionModel.name', newVal, oldVal);
      },
      immediate: true
    }
  }
