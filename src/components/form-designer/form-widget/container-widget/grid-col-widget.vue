...
export default {
  ...
  watch: {
    'designer.formConfig.layoutType': {
      handler(val) {
        this.updateLayoutProps(val, this.widget.options.responsive);
      }
    },
    'widget.options.responsive': {
      handler(val) {
        this.updateLayoutProps(this.designer.formConfig.layoutType, val);
      }
    },
    'widget.options.span': {
      handler(val) {
  ...
  methods: {
    initLayoutProps() {
      this.updateLayoutProps(this.designer.formConfig.layoutType, this.widget.options.responsive);
    },
    updateLayoutProps(layoutType, responsive) {
      if (responsive) {
        if (layoutType === 'H5') {
